...
api.MapGet("/views", async (ISavedViewRepository savedViewRepository, ClaimsPrincipal user, HttpContext context, CancellationToken ct) =>
{
    var username = user.Identity?.Name ?? string.Empty;
    var includeAll = HasScope(context.User, "admin.users");
    var views = await savedViewRepository.GetVisibleAsync(username, includeAll, ct);
    return Results.Ok(views);
}).RequireAuthorization(Policies.ScriptsRead);

api.MapPost("/views", async (SavedViewUpsertRequest request, ISavedViewRepository savedViewRepository, ClaimsPrincipal user, HttpContext context, CancellationToken ct) =>
{
    if (string.IsNullOrWhiteSpace(request.Name) || string.IsNullOrWhiteSpace(request.DefinitionJson))
    {
        return Results.ValidationProblem(new Dictionary<string, string[]>
    ...
    var owner = user.Identity?.Name ?? string.Empty;
    var saved = await savedViewRepository.UpsertAsync(new SavedViewUpsert(
        request.Id,
        request.Name,
        visibility,
        request.DefinitionJson,
...
api.MapPatch("/views/{id:guid}", async (Guid id, SavedViewUpsertRequest request, ISavedViewRepository savedViewRepository, ClaimsPrincipal user, HttpContext context, CancellationToken ct) =>
{
    if (string.IsNullOrWhiteSpace(request.Name) || string.IsNullOrWhiteSpace(request.DefinitionJson))
    {
        return Results.ValidationProblem(new Dictionary<string, string[]>
...
api.MapDelete("/views/{id:guid}", async (Guid id, ISavedViewRepository savedViewRepository, ClaimsPrincipal user, HttpContext context, CancellationToken ct) =>
{
    var deleted = await savedViewRepository.DeleteAsync(id, user.Identity?.Name ?? string.Empty, HasScope(context.User, "admin.users"), ct);
    return deleted ? Results.NoContent() : Results.NotFound();
}).RequireAuthorization(Policies.ScriptsRead);

api.MapGet("/admin/views", async (ISavedViewRepository savedViewRepository, CancellationToken ct) =>
{
    var views = await savedViewRepository.GetVisibleAsync(string.Empty, includeAll: true, ct);
    var result = views.Select(x => new { x.Id, x.Name, x.OwnerUsername, x.Visibility, x.UpdatedUtc });
    return Results.Ok(result);
}).RequireAuthorization(Policies.AdminUsers);
...
internal sealed record SavedViewUpsertRequest(
    Guid? Id,
    string Name,
    string? Visibility,
    string DefinitionJson);
